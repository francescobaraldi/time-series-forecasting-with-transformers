...
def test_transformer_multistep(device, model, dl, forecast_len, scaler, max_num=50, save_path=None):
    model = model.to(device)
    model.eval()
...
def test_lstm(device, model, dl, forecast_len, scaler, max_num=50, save_path=None):
    ...
    with torch.no_grad():
        ...
        for j, (input, window_len, class_idx) in enumerate(dl):
            ...
            plot_predictions(src[0:1, :, :].cpu(), trg[0:1, :, :].cpu(), prediction[0:1, :, :].cpu(), scaler, forecast_len, class_idx, j,
                             save_path)
