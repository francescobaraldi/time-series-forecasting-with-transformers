...
class YahooDataset2(Dataset):
    ...
    def __getitem__(self, index):
        src = self.dataset[index:index + self.window_len + 1, :]
        trg_forecast = self.dataset[index + self.window_len: index + self.window_len + self.forecast_len, :]
        src = self.scaler.transform(src)
        trg_forecast = self.scaler.transform(trg_forecast)
        return torch.from_numpy(src).to(torch.float32), torch.from_numpy(trg_forecast).to(torch.float32), self.class_idx


class YahooDatasetStd(Dataset):
    def prepare_dataset(self, dataset_path):
        dataset = pd.read_csv(dataset_path)
    ...
    def __getitem__(self, index):
        src = self.dataset[index:index + self.window_len + 1, :]
        index += self.forecast_len
        trg_forecast = self.dataset[index: index + self.window_len, :]
        src = self.scaler.transform(src)
        trg_forecast = self.scaler.transform(trg_forecast)
        return torch.from_numpy(src).to(torch.float32), torch.from_numpy(trg_forecast).to(torch.float32), self.class_idx
