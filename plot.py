...
def plot_predictions(src, trg, predictions, scaler, class_idx, i, save_path=None):
    ...
    trg_rec = scaler.inverse_transform(trg[0, :, :].cpu())
    predictions_rec = scaler.inverse_transform(predictions[0, :, :].cpu() + torch.zeros((n, input_size)))
    src_rec = src_rec[:, class_idx].tolist()
    trg_rec = trg_rec[:, class_idx].tolist()
    predictions_rec = predictions_rec[:, 0].tolist()
    plt.figure(figsize=(15,6))
    plt.plot(np.arange(1, window_len + 1), src_rec, '-', color='green', label='Source', linewidth=2)
    plt.plot(np.arange(1 + window_len, window_len + forecast_len + 1), trg_rec, '-', color='blue', label='Target', linewidth=2)
    plt.plot(np.arange(1, window_len + forecast_len), predictions_rec, '--', color='red', label='Prediction', linewidth=2)
    plt.xlabel("Days")
...
def plot_predictions2(src, trg, predictions, scaler, forecast_len, class_idx, i, save_path=None):
    ...
    src_rec = scaler.inverse_transform(src[0, :, :].cpu())
    trg_rec = scaler.inverse_transform(trg[0, :, :].cpu())
    predictions_rec = scaler.inverse_transform(predictions[0, :, :].cpu() + torch.zeros((window_len, input_size)))
    src_rec = src_rec[:, class_idx].tolist()
    trg_rec = trg_rec[:, class_idx].tolist()
    predictions_rec = predictions_rec[:, 0].tolist()
    plt.figure(figsize=(15,6))
